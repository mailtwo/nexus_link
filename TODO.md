...
- [13] 멀티 윈도우 알파 구현 목표 창 구현(우선순위): 월드 맵+네트워크 트레이싱, topology viewer, 파일 전송 대기줄, 웹페이지 뷰어(CEF), 프로세서 목록 (`plans/13_multi_window_engine_contract_v1.md` §12.5).
- [13] 코딩 에디터 윈도우(단일 창 + 탭 기반 다중 문서) 추후 구현 (`plans/13_multi_window_engine_contract_v1.md` §7.3, §13).
- [13] SSH_LOGIN `connect` 결과 기반 UI 효과(성공/실패 색상/애니메이션) 추가 구현 (`plans/13_multi_window_engine_contract_v1.md` §9.2, §9.4).
