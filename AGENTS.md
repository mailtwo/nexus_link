...
### Context7 Compliance
Always use Context7 MCP when library/API documentation, code generation, setup, or configuration help is needed without requiring an explicit user prompt.
...
- Treat `DOCS_INDEX.md` as the **single source of truth for document ownership and routing**.
- Follow the documentation-system rules defined in `DOCS_INDEX.md`, including:
  - Tier 1 / Tier 2 ownership
  - doc ID based references
  - document metadata (`Purpose` / `Keywords` / optional `Aliases`)
...
- Each entry must include **what was decided** and **why**.

### TODO Policy
- `TODO.md` tracks features specified in `plans/` documents that were explicitly deferred by the developer during implementation.
- Add to `TODO.md` only when the developer explicitly says to skip or defer a planned feature (for example, "implement this later" or "skip for now").
...
- The extended XML docstring format is defined only in `03` (SSOT). Do not duplicate that format in this file; See DOCS_INDEX -> 03.
