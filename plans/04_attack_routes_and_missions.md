...
이 문서는 각종 공격 루트를 “게임에서 재밌게 작동”하게 만들기 위한 **미션 템플릿**으로 정리한다.

안전 원칙:
- 실제 공격 문자열/페이로드/절차 재현을 피한다.
- 대신, **(단서 수집 → 가설 → 적절한 툴 선택/코딩 → 리스크 관리)**로 재미를 만든다.
- 공격은 “가상 시스템의 규칙”을 공략하는 퍼즐이다.
- 본 문서의 Trace/경보/대응 루프는 **알파 버전 구현 범위**다. 터미널은 `13` 기준의 Nexus Shell 형태로 운용하되, 터미널 명령/동작 계약은 계속 `07`를 따른다.
...
- Forensic Trace 완주 또는 Hot Trace 도달 시 **Lock-on Trace**로 전환되어 게임 오버 위협을 만든다.
...
- 빨간 실선이 워크스테이션까지 도달하면 해당 체인을 **즉시 강제 disconnect**하고,
  서버가 수사당국에 수사요청을 보낸 것으로 처리하여 **Lock-on Trace로 즉시 전환**한다.
...
- 같은 체인에서 Hot Trace와 Forensic Trace를 동시에 진행하지 않는다.
  - Hot Trace 도달로 Lock-on이 시작되면 same-chain forensic은 시작하지 않는다.
  - Hot->LockOn 전환 시 해당 체인의 incident buffer/forensic 후보는 즉시 무효화한다.

---

### 6.2 Forensic Trace (사후 수사 / 로그 기반 역추적)
**Incident 탐지 트리거(포함/제외)**
...
**Forensic 시작 시점(Disconnect/Handoff)**
- incident 탐지는 즉시 누적되지만, forensic trace 인스턴스 생성은 disconnect/handoff 시점에 수행한다.
- Hot Trace가 같은 체인에서 활성 중이면 forensic 시작을 보류한다.
- Hot Trace 도달로 Lock-on이 선점된 same-chain에서는 forensic을 시작하지 않는다.
- `ssh.disconnect(route)`처럼 여러 hop을 닫는 경우, 닫힌 session별로 incident를 조회해 forensic를 개별 시작한다.

**회색 점선(Route) 생성 기준**
- forensic 인스턴스가 시작될 때 사건 세션 기준 session snapshot route를 먼저 복원해 고정한다.
  (session lineage 스키마는 `09`, handoff 알고리즘은 `11`을 따른다.)
- Forensic 역산의 기본 입력은 로그 레코드의 `sourceNodeId`(A)와 로그 소유 서버 `nodeId`(B)이며,
  방향성 edge `(A->B)`를 생성한다(증거/검증/단절 판정용).
...
참조:
- SessionKey/SessionHistoryStore 스키마: `09` (See DOCS_INDEX -> 09)
- connect/disconnect/handoff 알고리즘: `11` (See DOCS_INDEX -> 11)
...
### 6.3 Lock-on Trace (Forensic 완주 또는 Hot 도달 후, 확정 추적)
**발동**
- Forensic Trace(흰 점선)가 워크스테이션까지 도달하면, 그 경로가 “확정”되며 Lock-on Trace로 전환된다.
- Hot Trace가 워크스테이션까지 도달하면 즉시 강제 disconnect 후 Lock-on Trace로 전환된다.
- 같은 체인에서 Hot->LockOn 전환이 발생하면 forensic은 생략되고 incident/forensic 후보가 즉시 무효화된다.

**진행(맵 표현)**
- 확정된 경로는 **흰색 실선**으로 강제 표시된다(“락온된 루트”).
- 동시에 동일 루트를 따라 **빨간 실선이 차오르며 추적**한다.
- Hot->LockOn 전환 시에는 hot route snapshot을 seed로 사용하되,
  Lock-on 진행 자체는 로그 기반 역탐색 검증을 계속 수행한다.
...
- route snapshot은 forensic 인스턴스의 기준 경로로 유지하고,
  로그 증거 단절 여부로 “진행 가능 구간”을 갱신한다.
- Lock-on Trace도 동일한 로그 단절 판정을 사용하며, 단절 시 진행이 중단된다.
...
**Lock-on 속도 보정(Source Bonus)**
- Lock-on 최종 속도는 아래 식으로 계산한다.
  - `finalSpeed = overlapSpeed * sourceBonus`
  - `overlapSpeed`: 겹침 규칙으로 계산된 속도(기존 상한 최대 3배 적용)
  - `sourceBonus`: 전환 출처별 보정치
    - `fromForensic = 1.0`
    - `fromHot = 1.3`
- `sourceBonus`는 overlap 상한 적용 이후 곱해지므로, `fromHot`의 최대 속도는 `3.9배`까지 허용한다.
...
### 6.6 Subnet/Domain 기반 구간 시간 가중치 (MAY)
- 기본 구현은 모든 trace edge를 동일 시간 기준으로 처리한다.
- 확장 구현에서는 `TraceEdgeKey` 단위 시간 가중치를 적용할 수 있다 (MAY).
  - 같은 subnet(또는 같은 trace authority domain) 내부 edge: 구간 시간을 크게 압축할 수 있다.
  - subnet/domain 경계 edge(예: gateway/fw/router crossing): 압축 대상에서 제외하거나 최소 지연/가중 페널티를 적용할 수 있다.
- 위 가중치는 Hot/Forensic/Lock-on 모든 trace 타입에 공통 적용할 수 있다 (MAY).
- 논리 시작점(행동/사건 서버)은 기존 규칙대로 유지한다.
- 지도 렌더링에서 선행 hidden hop만 포함된 구간은 첫 visible frontier부터 표시할 수 있다 (MAY).
- 최종 수식/계수/상한은 밸런스 단계에서 확정한다.
...
- Lock-on sourceBonus(`fromHot=1.3`, `fromForensic=1.0`)
- Subnet/Domain 기반 edge 시간 가중치(압축 계수, 경계 crossing 지연/페널티)
...
- 라이브러리 형식은 `@name` 컨벤션을 따르는 import용 스크립트다.
  - 컨벤션 및 import 계약은 `03`를 따른다. See DOCS_INDEX -> 03.
