...
- **CampaignBlueprint**: 시나리오를 묶는 상위 단위  
  ...
  단, 스테이지의 상세 내용(플레이 흐름/연출/난이도 설계)은 `15`를 따른다.
  See DOCS_INDEX -> 15.
...
- 스테이지 상세 내용(플레이 흐름/연출/난이도 설계)은 `15`를 따른다.
  See DOCS_INDEX -> 15.
- 본 문서(10)의 범위:
...
- 인접 계산:
  ...
  - `net.scan` API/시스템콜 반환 형식은 `03` / `07`를 따른다.
