...
- `[미구현]` `rmdir <dir>`: 빈 디렉토리를 삭제하는 목표 명령이다. 현재 시스템콜로는 제공되지 않는다.
...
- `[미구현]` `echo <text>`: 입력 문자열을 그대로 출력하는 유틸 명령 목표 항목이다. 현재 시스템콜로는 제공되지 않는다.
- `[미구현]` `clear`: 터미널 화면 정리 유틸 명령 목표 항목이다. 현재 시스템콜로는 제공되지 않는다.

### 6.3 네트워크(가상)
- `[미구현]` `ping <host|ip>`: 실제 ICMP 대신 “가상 연결성/지연”을 출력하는 시뮬레이션 목표 항목이다. 현재 시스템콜로는 제공되지 않는다.
...
- 현재 구현 상태 메모: `ping` 시스템콜은 아직 미구현이므로 위 항목은 현 시점에서 부분 미충족 상태다.
