...
- `[구현됨]` `rm <path>`: 경로 엔트리를 삭제한다. 현재 구현은 파일 삭제만 지원한다(디렉토리 삭제 미지원).
- `[미구현]` `cp <src> <dst>`: 로컬 파일/디렉토리 복사 명령 목표 항목이다. 현재 시스템콜로는 제공되지 않는다.
- `[미구현]` `mv <src> <dst>`: 로컬 파일/디렉토리 이동(이름 변경 포함) 명령 목표 항목이다. 현재 시스템콜로는 제공되지 않는다.
