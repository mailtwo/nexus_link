...
단, 실제 split 조절 범위는 현재 표시 중인 pane의 constraint system에 의해 더 좁게 제한될 수 있다.
...
- **DisplayedPane**: `DOCKED` 상태에서 `MaximizedPane` 제외와 fallback 규칙까지 적용한 뒤, 특정 slot에 실제로 렌더링되는 pane 1개 또는 `null`.
...
- **MinUsableWidth**: pane이 가로 축에서 의미 있게 표시되기 위해 필요한 최소 usable width.
- **MinUsableHeight**: pane이 세로 축에서 의미 있게 표시되기 위해 필요한 최소 usable height.
- **HorizontalResolvePolicy**: pane이 가로 축 minimum 이하로 줄어들려 할 때 적용하는 해결 정책.
- **VerticalResolvePolicy**: pane이 세로 축 minimum 이하로 줄어들려 할 때 적용하는 해결 정책.
- **BranchConstraint**: split tree의 한 branch가 현재 표시 중인 pane 제약을 합성해 계산한 축별 minimum / resolve 결과.
...
`DOCKED` 상태에서 `ActiveDockPane`는 기본 표시 대상으로 취급된다.
단, 실제 렌더링은 `DisplayedPane` 규칙을 따른다.

### 12.3 DisplayedPane
각 dock slot은 `DisplayedPane`(pane 1개 또는 `null`)를 가진다.
`DisplayedPane`은 `DOCKED` 상태에서 실제로 렌더링되는 pane을 뜻한다.

일반적으로는 `DisplayedPane == ActiveDockPane`이지만, 항상 같지는 않다.
예:
- `MaximizedPane` 제외 규칙 때문에 `ActiveDockPane`가 docked rendering 대상에서 빠진 경우
- 해당 slot이 `DockStack` 내부 fallback pane을 표시한 경우
- fallback 가능한 pane이 없어 empty slot을 표시하는 경우

### 12.4 Resident / Visible / Hidden
...
### 13.4 DOCKED 표시 규칙
`DOCKED` 상태에서는 각 slot의 `ActiveDockPane`를 기준으로 `DisplayedPane`을 계산해 표시한다.
...
## 15. Pane Constraint System

### 15.1 목적
Pane Constraint System은 pane별 최소 usable size와, 해당 minimum 아래로 더 줄어들려 할 때의 축별 해결 정책을 정의한다.

이 시스템의 목적은 아래 둘을 동시에 만족시키는 것이다.
- pane이 정보 의미를 잃을 정도로 무의미하게 작아지는 것을 막는다
- split 조절과 pane 내부 overflow 처리를 일관된 규칙으로 분리한다

### 15.2 평가 대상
constraint는 현재 **실제로 표시 중인 pane** 기준으로 평가한다.

- `DOCKED`: 각 slot의 `DisplayedPane`
- `MAXIMIZED`: split-level constraint는 평가하지 않으며, pane 내부 overflow 정책만 의미를 가진다

empty dock slot은 pane-specific constraint를 제공하지 않는다.

### 15.3 알파의 resolve policy
알파에서 축별 resolve policy는 아래 둘만 canonical로 허용한다.

- `CLAMP`
- `SCROLL`

정의:
- `CLAMP`: 해당 축의 split 조절을 더 이상 허용하지 않는다
- `SCROLL`: split 조절은 계속 허용하되, pane 내부 overflow/scroll로 해결한다

### 15.4 split floor와 pane-local scroll threshold
본 문서는 **split floor**와 **pane-local scroll threshold**를 분리한다.

- `CLAMP` pane의 minimum은 split 최소치 계산에 참여한다
- `SCROLL` pane의 minimum은 split을 멈추는 기준이 아니라, 해당 pane 내부 scroll이 켜지는 threshold다

즉 policy 충돌 시 규칙은 아래로 고정한다.
- split 최소치 결정: `CLAMP` 우선
- split 허용 후 pane 처리: `SCROLL` pane은 내부 scroll로 해결

### 15.5 branch 합성 규칙
split은 pane 개별 제약을 직접 보지 않고, split tree의 branch 단위로 constraint를 합성해 계산한다.

같은 축에서 branch minimum은 아래로 계산한다.
- 해당 branch에 현재 표시 중인 pane들 중
- 같은 축에서 `CLAMP` policy를 가진 pane들의 minimum만 모은 뒤
- 그 값의 `max(...)`

따라서 같은 branch 안에 여러 pane이 동시에 존재하면, 더 강한 minimum이 branch floor가 된다.

만약 해당 축에서 `CLAMP` pane이 하나도 없다면, 그 branch는 그 축에서 split floor를 제공하지 않는다.

### 15.6 shared branch 예시
알파의 대표 예시는 아래와 같다.

#### A. `LEFT <-> RIGHT` 가로 split
가로 split은 아래 3개의 현재 표시 pane 제약을 함께 본다.
- `LEFT`의 `DisplayedPane`
- `RIGHT_TOP`의 `DisplayedPane`
- `RIGHT_BOTTOM`의 `DisplayedPane`

여기서 오른쪽 branch의 가로 minimum은 `RIGHT_TOP` / `RIGHT_BOTTOM` 중 가로 축에서 `CLAMP` policy를 가진 pane minimum의 `max(...)`로 계산한다.

#### B. `RIGHT_TOP <-> RIGHT_BOTTOM` 세로 split
세로 split은 아래 2개의 현재 표시 pane 제약을 함께 본다.
- `RIGHT_TOP`의 `DisplayedPane`
- `RIGHT_BOTTOM`의 `DisplayedPane`

여기서 위/아래 branch의 세로 minimum은 각각 해당 branch의 현재 표시 pane 중 세로 축 `CLAMP` minimum으로 계산한다.

### 15.7 알파 pane별 canonical constraint
알파에서 constraint가 확정된 pane은 아래와 같다.

#### `TERMINAL`
- `MinUsableWidth = 150px`
- `MinUsableHeight = 100px`
- `HorizontalResolvePolicy = CLAMP`
- `VerticalResolvePolicy = SCROLL`

#### `WORLD_MAP_TRACE`
- reference texture size는 현재 `2048x1024`를 사용한다
- `MinUsableWidth = reference texture width의 10%`
- `MinUsableHeight = reference texture height의 10%`
- `HorizontalResolvePolicy = CLAMP`
- `VerticalResolvePolicy = CLAMP`

현재 알파 기준 유효 minimum은 아래와 같다.
- 가로 `205px`
- 세로 `103px`

percentage-derived minimum은 소수점 계산 후 `ceil` 반올림으로 고정한다.

### 15.8 아직 canonical constraint가 없는 pane
다른 pane kind는 이번 문서 버전에서 구체 constraint를 아직 canonical로 정의하지 않는다.

reserved rule:
- pane kind가 split/overflow 구현에 참여하려면, 구현 전에 해당 pane의 축별 minimum과 resolve policy를 먼저 본 문서에 추가해야 한다
- 구현이 문서보다 앞서가서는 안 된다

---

## 16. Pane open / focus / close / move 규칙
...
### 19.5 2단계 hydrate/sanitize 코어 메모
...
### 19.6 Pane Constraint System 구현 메모
Pane Constraint System 구현에서는 runtime이 아래 개념을 분리해 가져야 한다.

- pane kind별 constraint registry
- split tree 기준 branch constraint resolver
- 축별 split clamp / pane-local scroll fallback

추가 구현 메모:
- `SCROLL` minimum은 split floor 계산에 참여하지 않는다
- split floor는 현재 표시 중인 pane의 `CLAMP` minimum만으로 계산한다
- constraint 평가는 resident pane 전체가 아니라 `DisplayedPane` 기준으로 수행한다
...
- split 조절은 현재 표시 중인 pane constraint system을 존중해야 한다.
- shared branch 충돌 시 split floor는 `CLAMP` 기반 branch minimum으로 계산되어야 한다.
- `TERMINAL`은 가로 축에서 split clamp, 세로 축에서는 pane 내부 scroll로 해결해야 한다.
- `WORLD_MAP_TRACE`는 가로/세로 축 모두 split clamp로 해결해야 한다.
...
### 21.10 DOCKED에서 MaximizedPane 제외
